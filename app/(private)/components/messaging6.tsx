...
const Page = () => {
  ...
  const handleClose = () => {
    setShowPopup(false);
    router.push("/dashboard/messaging"); // navigate after closing
  };

  return (
    <>
      {showPopup && (
        <div className="relative w-full h-screen bg-[#18181B1F]">
          <div className="absolute inset-0 z-0">{/* Background layer */}</div>
          ...
          <section className="w-full min-h-screen flex justify-center items-center px-4 py-8">
            <div className="relative z-10 w-full max-w-[717px] rotate-0 opacity-100 rounded-[16px] border border-solid bg-[#ffffff] border-[#E2E4E84D]">
              ...
              <section className="w-full h-[60px] flex justify-between items-center rounded-t-[16px] border-b border-[#F6F6F6] px-[20px] py-[12px] bg-[#fff]">
                ...
                <button
                  onClick={handleClose} // close & navigate
                  className="w-[36px] h-[36px] flex items-center justify-center rounded-full border bg-[#F6F8FA] border-[#F1F2F3]"
                >
                  <Icon
                    icon="charm:cross"
                    width="20"
              ...
              <section className="w-full flex flex-col px-4 sm:px-[64px] py-[32px] gap-[32px]">
                ...
                <div className="flex w-full justify-end gap-[32px]">
                  <button
                    onClick={handleClose} // cancel closes & navigates
                    className="w-[161px] h-[40px] rounded-lg bg-[#EAECF0] px-[16px] font-semibold text-sm leading-5 text-center font-sans text-[#1D2939]"
                  >
                    Cancel
                  </button>
