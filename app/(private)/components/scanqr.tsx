...
// import React, { useEffect, useRef, useState } from 'react';
// import { Icon } from '@iconify/react';
// import { RunAgentInfo } from '@/app/Apis/publicapi';

// interface ScanqrpageProps {
//   setScanopen: (val: boolean) => void;
//   space_id: string;
//   space_uuid?: string; // Add this prop to receive UUID from parent
// }

// const Scanqrpage = ({ setScanopen, space_id, space_uuid }: ScanqrpageProps) => {
//   const [qrImage, setQrImage] = useState<string | null>(null);
//   const [loading, setLoading] = useState(true);
//   const [errorMsg, setErrorMsg] = useState<string | null>(null);
//   const [isLinked, setIsLinked] = useState(false);
...
// const fetchQrImage = async () => {
//   if (!space_uuid || !space_id) {
//     setErrorMsg("Missing space UUID or ID.");
//     return;
//   }
...
//   try {
//     const response: any = await RunAgentInfo(space_uuid);

//     // Check if response is blob (QR image)
//     if (response?.data instanceof Blob) {
//       // Detect if response is JSON (already linked) instead of QR image
//       const contentType = response?.headers?.['content-type'];
...
//       if (data.linked === true || data._whatsapp_linking_status_ === 1) {
//         setIsLinked(true);
//         setErrorMsg(null);
//         setQrImage(null);
//       } else {
